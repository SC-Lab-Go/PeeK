#ifndef _PEEK_STATUS_ARRAY_H
#define _PEEK_STATUS_ARRAY_H
...
class PeeKStatusArray : public KSP<GraphType, num_threads, pps> {
   ...
   public:
    explicit PeeKStatusArray(const GraphType& g, const unsigned int k) noexcept : ParentType(g, k), reverse_graph(GraphType::is_directed ? std::make_unique<KSPGraph<GraphType>>(g.get_reverse_graph()) : nullptr), reverse_sssp_t(g.get_num_nodes()), normal_sssp_t(g.get_num_nodes()) {
    ...
    ~PeeKStatusArray() { delete[] is_k_bound_nodes; }
