#ifndef _PEEK_REGENERATION_H
#define _PEEK_REGENERATION_H
...
class PeeKRegeneration : public KSP<GraphType, num_threads, pps> {
   ...
   public:
    explicit PeeKRegeneration(const GraphType& g, const unsigned int k) noexcept : ParentType(g, k), reverse_graph(GraphType::is_directed ? std::make_unique<KSPGraph<GraphType>>(g.get_reverse_graph()) : nullptr), reverse_sssp_t(g.get_num_nodes()), normal_sssp_t(g.get_num_nodes()) {
    ...
    ~PeeKRegeneration() { delete[] is_k_bound_nodes; }
