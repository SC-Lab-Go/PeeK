#ifndef _PEEK_ADAPTIVE_WITH_STATUS_ARRAY_H
#define _PEEK_ADAPTIVE_WITH_STATUS_ARRAY_H
...
class PeeKAdaptiveWithStatusArray : public KSP<GraphType, num_threads, pps> {
   ...
   public:
    explicit PeeKAdaptiveWithStatusArray(const GraphType& g, const unsigned int k) noexcept : ParentType(g, k), reverse_graph(GraphType::is_directed ? std::make_unique<KSPGraph<GraphType>>(g.get_reverse_graph()) : nullptr), reverse_sssp_t(g.get_num_nodes()), normal_sssp_t(g.get_num_nodes()) {
    ...
    ~PeeKAdaptiveWithStatusArray() { delete[] is_k_bound_nodes; }
    ...
    std::vector<Path> compute(const NODE_ID source, const NODE_ID destination) {
        ...
        if (k_bound_nodes_num > adaptive_ratio * orig.get_num_nodes()) {
            OptYen<DeltaSteppingStatic, GraphType, num_threads, COMPACT_KSP_PD> optyen(orig.get_original_graph(), k, reverse_sssp_t, paths, is_k_bound_nodes);
...
#endif
            result = optyen.compute(source, destination, false);
            // optyen.print_paths();
            return result;
        }
...
#endif  // _PEEK_ADAPTIVE_WITH_STATUS_ARRAY_H