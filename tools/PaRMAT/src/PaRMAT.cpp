...
void openFileToWrite( std::ofstream& input_file, const char* file_name ,unsigned long long nVertices) {
	input_file.open(file_name);
	input_file << nVertices << "\n";
	if( !input_file )
	...
	try{
		...
		for( int iii = 1; iii < argc; ++iii ) {
			if( !strcmp(argv[iii], "-nEdges") && iii != argc-1 /*is not the last one*/)
				...
				openFileToWrite( outf, argv[iii+1], nVertices );
		...
		if( !outf.is_open() )
			openFileToWrite( outf, "out.txt", nVertices );
